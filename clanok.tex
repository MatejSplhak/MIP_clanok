...
Na kompresiu si tvorcovia vybrali H.264, pretože to je najrozšírenejší štandard na kompresiu videa a x264 pretože to je najpopulárnejší video encoder. Pri kompresii sú predpovedané mapy významnosti downscalenuté aby pasovali do rozmerov 16x16 macroblokovej mriežky. Ak \(Q ∈ R^+\) je mapa hodnôt bitratu macroblocku vybraných encoderom, a \(S:R^2 → [0; 1]\) je downscalnutá mapa významnosti pre daný snímok tak nové hodnoty biratu môžu byť vypočitané nasledujúcou rovnicou:
